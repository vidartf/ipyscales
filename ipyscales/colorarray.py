...
def minlen_validator(minlen):
    def validator(trait, value):
        if value.shape[0] < minlen:
            raise TraitError(
                "%s needs have a minimum length of %d, got %d"
...
def color_array_shape_validator(trait, value):
    ...
    if len(value.shape) != 2:
        raise TraitError(
            "%s shape expected to have 2 components, but got shape %s"
            % (trait.name, value.shape)
        )
    if not (3 <= value.shape[-1] <= 4):
        raise TraitError(
            "Expected %s to have 3 or 4 elements for the last dimension of its shape, but got %d"
...
class ArrayColorScale(SequentialScale, ColorScale):
    ...
    space = Enum(["rgb", "hsl"], "rgb", help="The color space of the range.").tag(
        sync=True
    )

    gamma = CFloat(1.0, help="Gamma to use if interpolating in RGB space.").tag(
        sync=True
    )
